...
module Audio
  ...
  def bgm_fade(time)
    return unless @instance[:bgm].is_a?(Gosu::SampleInstance)
    Thread.new {
      incs = self.bgm_volume / time
      until self.bgm_volume <= 0
        self.bgm_volume -= incs
        sleep 0.01
      end
      bgm_stop
    }
...
#=============================================================================
  ...
  def bgs_fade(time)
    return unless @instance[:bgs].is_a?(Gosu::SampleInstance)
    Thread.new {
      incs = self.bgs_volume / time
      until self.bgs_volume <= 0
        self.bgs_volume -= incs
        sleep 0.01
      end
      bgs_stop
    }
...
#=============================================================================
  ...
  def me_fade(time)
    return unless @instance[:me].is_a?(Gosu::SampleInstance)
    Thread.new {
      incs = self.me_volume / time
      until self.me_volume <= 0
        self.me_volume -= incs
        sleep 0.01
      end
      me_stop
